...
public class SortOld {
    ...
    private static void mergeSort(int[] toSort, int s, int e, int[] tmp, BufferedWriter bw, int lvl) throws IOException
    {
        ...
        {
            int m = ((e-s)/2+s);
            System.out.println("calling mergeSort(toSort, "+s+", "+m+", tmp, bw,"+(lvl+1)+");");
            mergeSort(toSort, s, m, tmp, bw, lvl+1);
            System.out.println("calling mergeSort(toSort, "+(m+1)+", "+e+", tmp, bw,"+(lvl+1)+");");
            mergeSort(toSort, m+1, e, tmp, bw, lvl+1);
